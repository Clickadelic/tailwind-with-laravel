<form action="{{ isset($movie) ? route('movies.update', $movie) : route('movies.store') }}" method="POST" class="w-96 mx-auto flex flex-col space-y-3 mb-12">
    @csrf
    @if(isset($movie))
        @method('PUT')
    @endif
    ...
    <div>
        <input type="number" 
               name="released" 
               value="{{ old('released', $movie->released ?? '') }}" 
               min="1940" max="2050"
               class="w-full px-3 py-2 border border-slate-400 rounded bg-white" 
